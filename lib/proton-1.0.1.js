...
//the own renderer
	...
	Renderer.prototype = {
		...
		getRenderer : function() {
			switch(this.type) {
				...
				case 'phaser':
					return new Proton.PhaserRender(this.proton, this.element);
					break;
	...
	function PhaserRender ( proton, element, stroke ) {
		
		PhaserRender._super_.call( this, proton, element );
		this.stroke = stroke;
	}

	Proton.Util.inherits( PhaserRender, Proton.BaseRender );

	PhaserRender.prototype.PI_180 = Math.PI / 180;

	PhaserRender.prototype.resize = function ( width, height ) {

	};

	PhaserRender.prototype.start = function () {
		PhaserRender._super_.prototype.start.call( this );
	};

	PhaserRender.prototype.onProtonUpdate = function () {

	};

	PhaserRender.prototype.setStroke = function(color, thinkness) {
		...
		//console.log('foobar', this.stroke);
	}

	/**
	 * @param particle
	 */
	PhaserRender.prototype.onParticleCreated = function ( particle ) {

		//console.log('dip')
		if ( particle.target ) {

			particle.target = this.element.game.make.sprite( particle.target );
			...
			var graphics = this.element.game.make.graphics();

			
			//console.log(this.stroke.color)
			if ( this.stroke ) {
				graphics.lineStyle(this.stroke.thinkness, this.stroke.color.replace('#', '0x'), 1 );
			}


			graphics.beginFill( particle.color.replace('#', '0x') );
			graphics.drawCircle( 0, 0, particle.radius*2 );
	 ...
	 * @param particle
	 */
	PhaserRender.prototype.onParticleUpdate = function ( particle ) {
		...
		if ( particle.target ) {
			...
			//console.log('derp', particle.target)
		}
	};
	 ...
	 * @param particle
	 */
	PhaserRender.prototype.onParticleDead = function ( particle ) {
		if ( particle.target ) {
			this.element.removeChild( particle.target );
		}
	};

	Proton.PhaserRender = PhaserRender;
